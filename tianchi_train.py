...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 22 10:35:35 2018
...
Data = pd.read_csv('/Users/zhayiqiang/Downloads/tianchi_train/DataSet/drop1112_sub_item.csv') #189万行
...
#        train_user_window1 = Data[(Data['daystime'] > (LabelDay - datetime.timedelta(days=FEATURE_EXTRACTION_SLOT))) & (Data['daystime'] < LabelDay)]
        ...
        #189万数据，根据12月18日做计算
        # user_id    item_id  item_category  label四个维度数据，共29749条数据
        #x.describe()
        #x.groupby('label').count() 
        #beforeoneday [68122 rows x 9 columns]
        add_user_click_1 = user_click(beforeoneday) #4132 rows x 96 columns 用户行为
        add_user_item_click_1 = user_item_click(beforeoneday) #29749 rows x 96 columns 商品
        add_user_cate_click_1 = user_cate_click(beforeoneday) #[10827 rows x 96 columns] 商品类别
        # add_user_click_2 = user_click(beforetwoday)
        # add_user_click_5 = user_click(beforefiveday)
        liveday = user_liveday(train_user_window1) #[14175 rows x 4 columns]
        # sys.exit()
